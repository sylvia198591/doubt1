...
class SignUpView(View):
    ...
    template_name = 'Userdetail/signup.html'
    model=User
...
class ActivateAccount(View):
    ...
    def get(self, request, uidb64, token, *args, **kwargs):
        ...
        if user is not None and account_activation_token.check_token(user, token):
            user.is_active = True
            user.cuser.email_confirmed = True
            user.cuser.accno = request.session['accno']
            print("aa",request.session['accno'])
            cuser.save(update_fields=user.cuser.accno)
            login(request, user)
